...
async function generateHTML1(rData1, label, rData2) {
  ...
  htmlContent += `var chart1 = new google.visualization.PieChart(document.getElementById('chart1'));
...
async function httpsInvoke(options = {}) {
  ...
  try {
    const response = await axios(custom, {
...
async function replaceThrottleRequest(up, down, deviceId) {
  ...
  try {
    await Object.keys(payload1).forEach(function (key) {
      if (
        typeof payload1[key].down != "undefined" &&
        typeof payload1[key].up != "undefined"
      ) {
...
async function getCUJDetails(sessionId) {
  const url =
    "https://ad1f809d87764b90a0a9a4bad0294825@api-dev.headspin.io/v0/sessions/" +
    sessionId +
    "/label/list?label_type=page-load-result&label_type=user";
  try {
...
async function generateHTML(sessionTag, sessionTag1) {
  ...
  let url =
    "https://ff60cdf18dc841559ada504885bc6118@api-dev.headspin.io/v0/sessions/" +
    `${sessionTag}` +
    ".csv";
  ...
  for (let [key, value] of map) {
    ...
    for (const [key, value] of tmap) {
      ...
      if (parseInt(key) > start && endTime < end) {
        if (
          (url.includes("discomax") ||
            url.includes("max-tests.com") ||
            url.includes("max-next.com") ||
            url.includes("hbomaxcdn") ||
            url.includes("cdn") ||
            url.includes("CDN")) &&
          !url.includes("events") &&
          ...
          !url.includes("google") &&
          !url.includes("fls.doubleclick.net") &&
          !url.includes("twitter") &&
          !url.includes("https://t.co") &&
          !url.includes("pug000") &&
          !url.includes("cdn.cookielaw") &&
          !url.includes("yahoo") &&
          !url.includes("json")
        ) {
          count++;
  ...
  const result = await getClientSideLatencyInSeconds(
    new Map(seconds1),
    new Map(rData3)
  );
  const result1 = await getClientSideLatencyInSeconds(
    new Map(seconds2),
    new Map(rData4)
  );

  const fResult = await getConsolidateLatency(
    new Map(seconds1),
    new Map(result)
  );
  const fResult1 = await getConsolidateLatency(
    new Map(seconds2),
    new Map(result1)
  );
  ...
  fs.writeFileSync(
    "output1.html",
    await generateHTML1(rData1, sessionTag, fResult),
    "utf-8"
  );
  fs.writeFileSync(
    "output2.html",
    await generateHTML1(rData2, sessionTag1, fResult1),
    "utf-8"
  );
  fs.writeFileSync(
    "output3.html",
    await generateHTML2(rData2, sessionTag1, sessionTag, rData1),
    "utf-8"
  );

  console.log(
    "***** The execution is completed. Please check the output1.html, output2.html and output3.html files for result *****"
  );
...
module.exports = {
  invokeAgentPostCall,
  replaceThrottleRequest,
  httpsInvoke,
  replaceLockDeviceRequest,
  profilerSwitch,
  jsonValidator,
  traversal,
  generateHTML,
